	...
	unsigned long max_high)
{
	...
	//  MAX_NR_ZONES  This file was generated by Kbuild  最后创建在 include/generated/bounds.h\
	// 这里有3个
	...
	for_each_memblock(memory, reg) {
		...
		if (start < max_low) {//0x8f800
			unsigned long low_end = min(end, max_low); // low_end = 0x8f800
			zhole_size[0] -= low_end - start;  //0
		}
#ifdef CONFIG_HIGHMEM
		if (end > max_low) {
			unsigned long high_start = max(start, max_low);//0x8f800
			zhole_size[ZONE_HIGHMEM] -= end - high_start;//0
		}
#endif
	}
