...
static int dup_mmap(struct mm_struct *mm, struct mm_struct *oldmm)
{
	...
	/* 如果父进程的mm_struct的flags设置了MMF_HAS_UPROBES，则子进程的mm_struct的flags
	设置MMF_HAS_UPROBES和MMF_RECALC_UPROBES */
	...
	/* 获取指向线性区对象的链表头，链表是经过排序的，按线性地址升序排列，但是mmap并不是list_head结构，
	是一个struct vm_area_struct指针，这里由于子进程的mm是刚创建的，mm->map为空，而pprev是一个指向指针的指针 */
	pprev = &mm->mmap;
	...
	prev = NULL;
	//  遍历父进程的进程地址空间vma/* 遍历父进程所有vma，通过mm->mmap链表遍历 */
	for (mpnt = oldmm->mmap; mpnt; mpnt = mpnt->vm_next) {
		struct file *file;
		/* mpnt指向父进程的一个vma */
		if (mpnt->vm_flags & VM_DONTCOPY) {/* 父进程的此vma标记了不复制 */
		...
		// 新建一个临时用的vm_area_struct/* 分配一个vma结构体用于子进程使用 */
		tmp = kmem_cache_alloc(vm_area_cachep, GFP_KERNEL);
		if (!tmp)
			goto fail_nomem;
		*tmp = *mpnt;/* 直接复制父进程的vma的数据到子进程的vma */

		//初始化tmp VMA中的anon_vma_chain 链表/* 初始化子进程新的vma的anon_vma_chain为空 */
		...
		*pprev = tmp;
		...
		// 把vma添加到子进程的红黑树 /* 将vma加入到mm的mm_rb这个vma红黑树中 */
		__vma_link_rb(mm, tmp, rb_link, rb_parent);
		rb_link = &tmp->vm_rb.rb_right;
		rb_parent = &tmp->vm_rb;

		mm->map_count++; /* 子进程mm的线性区个数++ */

		/* 做页表的复制
         * 将父进程的vma对应的开始地址到结束地址这段地址的页表复制到子进程中
         * 如果这段vma有可能会进行写时复制(vma可写，并且不是共享的VM_SHARED)，
         那就会对子进程和父进程的页表项都设置为映射的页是只读的(vma中权限是可写)，
         这样写时会发生缺页异常，在缺页异常中做写时复制 
         */
